...
__global__ void max_element(int *d_vec, int *d_seg, int num_segments, int* d_value) {
	...
	int n = (uint)log2((double)num_segments) + 1;
	//if(id >= d_seg[bx] && id < d_seg[bx + 1]) {
	for (int i = 0; i < n; i++) {
...
__global__ void pre_fix_sum(int *d_vec, int *d_seg, int num_segments, int* d_value) {
	...
	for (i = 0; i < num_of_segments; i++) {
		if(d_seg[bx] < d_seg[bx + 1]) {
			uint segIndex = bx << mostSignificantBit;
			h_vec[j] += segIndex;
		}
	}
